...
class PTMEncode(object):
    ...
    def encode_batch(self, texts):
        # 使用分词器对文本进行编码，得到输入ID、注意力掩码和类型ID
...
if __name__ == "__main__":
    ...
    # --------------------- for news --------------------- #
    simcse = PTMEncode(
        fname='News-Environment-Perception-main\\dataset\\{}\\news\\news.json'.format(dataset),
    ...
    # save all_vecs
    with open('News-Environment-Perception-main\\preprocess\\SimCSE\\data\\{}\\news\\all_vecs_origin.pkl'.format(dataset), 'wb') as f:
        pickle.dump(all_vecs, f)

    # --------------------- for post --------------------- #
    simcse = PTMEncode(
        fname='News-Environment-Perception-main\\dataset\\{}\\post'.format(dataset),
        PTM_path=SimCSE_ckpt,
        batch_size=256,
        max_length=MAX_LEN,
        device=device)
    ...
    for d in ['train', 'val', 'test']:
        with open('News-Environment-Perception-main\\preprocess\\SimCSE\\data\\{}\\post\\{}_vecs_origin.pkl'.format(dataset, d), 'wb') as f:
